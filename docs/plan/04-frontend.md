...
| ✚ `/accounting/documents` | **built (M9), then parked** (D93). Every file with what it is attached to, filtered by type, month and text. The owner does not need it; it stays as it is and gets no more work. The number that mattered — files attached to nothing — is the exception inbox's job | M9 |
