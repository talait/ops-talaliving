...
| ✚ `/procurement/approvals` | **the standing queue** — every submitted line nobody has decided, oldest first, grouped by the submission it arrived in. Per line: the purpose, an approved amount that starts at what was requested and may only be reduced, and **one checkbox**. Below it, *decided recently* with the append-only trail and **un-approve**. Hidden from the menu without `approve_goods` (D60); readable without the controls for anyone with `procurement.read` | M5 |
