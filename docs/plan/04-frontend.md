...
| ✚ `/accounting/documents` | **built (M9)**: every file with what it is attached to, filtered by type, month and text. Counts how many cover more than one record, and how many are attached to nothing — the ones that arrived and were never claimed | M9 |
