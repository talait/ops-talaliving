...
create table core.user_modules (  -- named as in 02-database.md
  user_id  uuid not null references core.users(id) on delete cascade,
  module   core.module_t not null,
  level    core.module_level_t not null,
...
create table core.user_authorities (
  user_id    uuid not null references core.users(id) on delete cascade,
  authority  core.authority_t not null,
  granted_by uuid references core.users(id),
  granted_at timestamptz not null default now(),
...
insert into core.permission_catalog (module, action, admin_only) values
...
create or replace function core.has_permission(code text)
returns boolean
...
create or replace function core.has_authority(a core.authority_t)
returns boolean
language sql stable security definer set search_path = core, pg_temp as $$
  select exists (
    select 1 from core.user_authorities ua
...
create or replace view core.v_my_access as
...
alter table core.user_modules    enable row level security;
alter table core.user_authorities enable row level security;
...
create policy catalog_read on core.permission_catalog
  for select to authenticated using (true);

-- Grants are readable by their owner and by whoever administers access.
create policy grants_read on core.user_modules
  for select to authenticated
  using (user_id = auth.uid() or core.has_permission('it.manage_roles'));
create policy authorities_read on core.user_authorities
  for select to authenticated
  using (user_id = auth.uid() or core.has_permission('it.manage_roles'));

-- Granting is `it.manage_roles`, and never self-service.
create policy grants_write on core.user_modules
  for all to authenticated
  using (core.has_permission('it.manage_roles'))
  with check (core.has_permission('it.manage_roles'));
create policy authorities_write on core.user_authorities
  for all to authenticated
  using (core.has_permission('it.manage_roles'))
  with check (core.has_permission('it.manage_roles'));
create policy users_write on core.users
  for all to authenticated
  using (core.has_permission('it.manage_users'))
  with check (core.has_permission('it.manage_users'));

grant usage on schema core to authenticated;
grant select on core.users, core.permission_catalog, core.user_modules,
                core.user_authorities, core.v_my_access to authenticated;
