...
| ✚ `/accounting/calendar` | **built (M12b)** — twelve months across, every recurring line down the side, planned above and actual below (`≈` when the actual was matched by category rather than named by a person). Carries the day each bill falls due, so the same list is the **reminder** at the top: overdue first, then the next three weeks, each with *link a payment* pointing at the ledger row that settled it. A cell opens *this month only*, which needs a reason. The **not in the plan** row is what keeps it honest (D111), and the verdict says the month the money runs out and by how much | M12b |
