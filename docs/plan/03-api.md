...
| GET | `/products/{product_code}/materials?qty=12&rev=2` | what a run of that size needs, **waste included**, with a total and a count of what could not be priced. A work order passes **its own pinned revision**; without `rev` the answer comes from the draft or the current released one (D256) |
