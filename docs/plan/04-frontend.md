...
| ▸ `/procurement/pr` (same board) | **Three groups, each with its own subtotal** (D71): waiting for approval (what is still to decide), approved and waiting for payment (what that will cost to pay), finished. A line paid without a yes sits with the undecided. **The decision, inline.** For the holder of `approve_goods`, every undecided row carries a quantity, an amount and one checkbox — change either and the other follows at the unit price; approval can only reduce. Above the table: BCA 271 against what is approved and unpaid, and the top-up that closes the gap. **Ask on Chat** sends the undecided rows to the approver. Instructions and remarks from leadership show on the row; un-approve and the append-only trail live in the drawer | M5 |
| ✚ `/demo/chat` | Google Chat, standing in for itself: **one card per send**, carrying the list and its three totals — asked for, approved so far, what has to be paid — plus the BCA 271 balance and the top-up if it falls short. Per item: quantity, amount, instructions, approve or not yet; and *approve the rest as asked* for what is left. Answering somebody else's list is refused, which is the point of the route (D69, D70) | M5b |
