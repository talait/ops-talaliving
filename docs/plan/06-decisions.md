...
| 67 | 2026-09-11 | **One board, not two.** Approving happens on the requests board, inline per row; the separate approval queue is deleted | owner. Approving is not a second subject — it is a column on the same list. Two screens meant two lists that could disagree about what is outstanding |
