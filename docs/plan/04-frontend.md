...
| ✚ `/procurement/rounds` | every live round (usually one, sometimes two — one being funded, one collecting): the four-state rail with what each state means, then requested · BCA 271 · to transfer · remaining after payment. **Roll in what is owed** (idempotent), **approve** (freezes the numbers, `approve_funds`), **record the transfer** (writes both ledger legs, `post_ledger`), **close** (which answers with the list of what it just released). A TRANSFERRED round shows every item still `WAITING FOR PAYMENT` — the proof that funding is not paying (A10) | M6 |
