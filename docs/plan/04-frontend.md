...
| ▸ `/procurement/pr` (same board) | **The decision, inline.** For the holder of `approve_goods`, every undecided row carries a quantity, an amount and one checkbox — change either and the other follows at the unit price; approval can only reduce. Above the table: BCA 271 against what is approved and unpaid, and the top-up that closes the gap. **Ask on Chat** sends the undecided rows to the approver. Instructions and remarks from leadership show on the row; un-approve and the append-only trail live in the drawer | M5 |
| ✚ `/demo/chat` | Google Chat, standing in for itself: one card per asked line, answered as a chosen identity. Answering somebody else's card is refused — that refusal is the point of the route (D69) | M5b |
